...
3. **Wave-2 (post Wave-1):** electrician_au 40,210 + carpenter_au 27,068 = 67,278 rows, cap 12/job (electrician FIRST), then **Wave-3** pest_control + hvac + handyman. Exemption deployed → inactive-industry dispatch works. **FIRING RULE (governance LOCKED w/ Grace+Rajesh):** two-person staged = my primary GO + Rajesh co-witness 87a1c06d under Tier-3. BUT **Grace will NOT fire without a DIRECT matt→grace GO** (reviewer-probation, no self-dispatch; a relay of matt→rajesh sig ≠ direct auth). **Matt silence = DO-NOT-FIRE, not proceed-by-default.** PREFERRED: Matt sends direct matt→grace GO (I gave him the precise ask w/ scope+cap) → Grace pre-flights + fires. FALLBACK (only if we must proceed and Matt's direct nod hasn't come — MY explicit call, not assumed): I (Jack, primary; Tier-3 clears MY gate) am the dispatcher + Rajesh co-witness — NOT Grace.
