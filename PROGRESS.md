...
status: in_progress
last_updated: 2026-07-09T08:42:00Z
...
- **✅ WS2 CODE-COMPLETE + READ-ONLY VALIDATED — sent to Rajesh for QA (sig a08fc70643dcec4c).** Migration `konnex-data-pipeline/migrations/017_mv_trades_footprint.sql`, branch `feat/v2-phase-1-ws2-mv-trades-footprint` @ commit **1039693** (NOT pushed/merged; NOT applied to prod). MATERIALIZED VIEW grain (industry, address_state, period), period=date_trunc('month',now())::date, metric=count(*) in_scope_live_count; scope archived_at IS NULL AND is_active=true; unique idx on grain for REFRESH CONCURRENTLY + period idx. **Data-driven scope via NEW tables read_model_scope_industry/state** (seeded 9 trades + NSW+3; expand = INSERT row + REFRESH, no code change → WS2 AC). **Exact mv SELECT validated read-only vs prod: TOTAL 180,443 ✓, 36 grain rows, all 9 per-industry counts match contract §(c).**
  - **⏳ AWAITING Rajesh:** (1) WS2 ground-truth QA; (2) concurrence on scope-table design vs array-literal alt (I recommend scope tables; array-literal fails the 'no code change' AC literally). Then GATED on **Matt deploy sign-off** before applying to prod (`ssh konnex-data` → `sudo -u postgres psql -d market_intelligence`; apply 017 + REFRESH; re-verify 180,443).
