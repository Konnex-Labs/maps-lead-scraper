...
status: in_progress
last_updated: 2026-07-09T08:36:00Z
...
- **✅ WS1 PUSHED — PR #26** https://github.com/Konnex-Labs/konnex-api/pull/26 (branch feat/v2-phase-1-read-models @ 6f28a34, off origin/main 04cfcdd). Suite 130/0. **AWAITING Rajesh WS1 QA on the branch before WS2.**
- **⏭️ NEXT: WS2** (mv_trades_footprint) — GATED on Rajesh WS1 QA clear. Needs prod DB (`ssh konnex-data` → `sudo -u postgres psql -d market_intelligence`) + Rajesh ground-truth QA vs 180,443. Grain (industry, address_state, period), unique idx for REFRESH CONCURRENTLY, parameterised on industries+states, align period with market_metrics (mig015). NOTHING to prod without Matt deploy sign-off.
